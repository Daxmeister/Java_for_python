...
public class lab5_implicit_snabbare {
    ...
    private static void pathfinder(CustomPair coordinates) {
        ...
        while (!queue.isEmpty()) {
            ...
            // Check that the current coordinate has not been visited before and that it is the correct letter
    ...
    private static void addIfInMatrix(CustomPair coordinates, Queue<CustomPair> queue) {
        ...
        if (rowCoord >= 0 && rowCoord < inputMatrix.length && colCoord >= 0 && colCoord < inputMatrix[0].length) {
            queue.offer(coordinates);
        }
    }
